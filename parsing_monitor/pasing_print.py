...
def prepare_comparison_data():
    ...
    input_folder = os.path.join(current_folder, 'parse_result') #папка содержания файлов парсинга
    assert os.path.exists('parse_result'),'Отсутствует папка parse_result, из которой собирается статистика'

    files = sorted(os.listdir(input_folder), reverse=True)  # список файлов
    data = ''
    for file in files:
        ...
        if extension == '.json':
            ...
            inp = pd.read_json(file_path, encoding='utf8')
...
def main(for_output):
    ...
    comparison_data=prepare_comparison_data()       #Результаты прошлых парснгов считанные из файлов
    for i in sorted(for_output, key=lambda x: x['Псевдоним']):
        comparison_val=statist_data(comparison_data,i['Псевдоним']) #статистика по каждой характиристике полученная из "старых" данных
